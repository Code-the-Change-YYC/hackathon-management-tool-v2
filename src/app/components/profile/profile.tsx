...
export default function Profile() {
  ...
  const formatProgramString = (value : string): string => value.split("_").map((value, _) => `${value.at(0)?.toUpperCase()}${value.substring(1)}`).join(" ")
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const firstName = formValues.get("firstName")?.toString() ?? null;
    const lastName = formValues.get("lastName")?.toString() ?? null;
    let program = formValues.get("major")?.toString() ?? null;
    const school = formValues.get("institution")?.toString() ?? null;

    //--Frontend checks--
    let passedChecks = true;
    if (firstName === null){ //Should never happen in this context
    ...
    if (lastName === null){ //Should never happen in this context
    ...
    if (program === null){ //Should never happen in this context
      toast.error("null found in field: Major")
      passedChecks = false
    } else if (program !== "Computer Science" && program !== "Software Engineering" && program !== "Electrical Engineering" && program !== "Other"){ //Should never happen in this context
      toast.error("Unknown value found in field: Major")
      passedChecks = false
    }
    if (school === null){ //Should never happen in this context
      toast.error("null found in field: Institution")
      passedChecks = false
    } else if (school !== "University of Calgary" && school !== "Mount Royal University" && school !== "SAIT" && school !== "Other"){ //Should never happen in this context
    ...
    //--Backend checks--
    program = program!.split(" ").map((value, _) => `${value.at(0)?.toLowerCase()}${value.substring(1)}`).join("_"); //Convert string to db format (ex. Computer Science => computer_science)
    
    if (`${firstName} ${lastName}` === session.user.name && program === session.user.program && school === session.user.school){ //No changes were actually made
      toast.success("Profile updated")
      setIsSubmitting(false);
      setIsEditing(false);
      return
    } else if (passedChecks){ //Passed front end checks
      updateUsers.mutate({
        id: session.user.id, 
        ...
        school: school as "University of Calgary"|"Mount Royal University"|"SAIT"|"Other",
        program: program as "computer_science"|"software_engineering"|"electrical_engineering"|"other" 
        //----------------------------------------------------------------
      })
    } else{
      setIsSubmitting(false);
        ...
        <main>
          <section className="relative flex items-center justify-between pb-[16px]">
            ...
            {isEditing && (
              ...
              >
                {isSubmitting && (
                  <LoadingLine className="animate-spin w-[95px]" color="#FFF"/>
                )}
                {!isSubmitting && (
                  <p
            ...
            <FieldGroup>
              <div className="inline-grid w-full grid-flow-row grid-rows-[repeat(2,fit-content(100%))] gap-x-[var(--spacing-9,48px)] gap-y-[24px] self-stretch rounded-[var(--radius-4,16px)] border border-[var(--grey-300,#D6D6D6)] border-solid p-[var(--spacing-6,24px)] sm:grid-cols-[repeat(2,minmax(0,1fr))]">
                <section>
                  ...
                  {isEditing && (
                    <>
                    <Field data-invalid={fNameError === null ? false : true}>
                      <Input aria-invalid={fNameError === null ? false : true} onChange={() => setFNameError(null)} className="text-[16px] md:text-[16px]" name="firstName" defaultValue={session?.user.fname ?? "John"}/>
                    </Field>
                    {fNameError && (
                      <FieldDescription>
                        {fNameError}.
                ...
                <section>
                  ...
                  {isEditing && (
                    <>
                    <Field data-invalid={lNameError === null ? false : true}>
                      <Input aria-invalid={lNameError === null ? false : true} onChange={() => setLNameError(null)} className="text-[16px] md:text-[16px]" name="lastName" defaultValue={session?.user.lname ?? "Doe"}/>
                    </Field>
                    {lNameError && (
                      <FieldDescription>
                        {lNameError}.
                ...
                <section className="truncate">
                  ...
                  {isEditing && (
                    <Select name="institution" defaultValue={session?.user.school ?? "University of Calgary"}>
                    <SelectTrigger className={`${fontStyles.bodyLargePrimary}self-stretch text-[var(--grey-800,#292929)] w-[100%]`}>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectGroup>
                        <SelectItem className={`${fontStyles.bodyLargePrimary}self-stretch text-[var(--grey-800,#292929)]`} value="University of Calgary">University of Calgary</SelectItem>
                        <SelectItem className={`${fontStyles.bodyLargePrimary}self-stretch text-[var(--grey-800,#292929)]`} value="Mount Royal University">Mount Royal University</SelectItem>
                        <SelectItem className={`${fontStyles.bodyLargePrimary}self-stretch text-[var(--grey-800,#292929)]`} value="SAIT">SAIT</SelectItem>
                        <SelectItem className={`${fontStyles.bodyLargePrimary}self-stretch text-[var(--grey-800,#292929)]`} value="Other">Other</SelectItem>
                      </SelectGroup>
                    </SelectContent>
                  </Select>
                  )}
                ...
                <section className="truncate">
                  ...
                  {!isEditing && (
                    <p
                      className={`${fontStyles.bodyLargePrimary}self-stretch text-[var(--grey-800,#292929)]`}
                    >
                      {(session && session.user.program) ? formatProgramString(session.user.program) : "Computer Science"}
                    </p>
                  )}
                  {isEditing && (
                    <Select name="major" defaultValue={(session && session.user.program) ? formatProgramString(session.user.program) : "Computer Science"}>
                    <SelectTrigger className={`${fontStyles.bodyLargePrimary}self-stretch text-[var(--grey-800,#292929)] w-[100%]`}>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectGroup>
                        <SelectItem className={`${fontStyles.bodyLargePrimary}self-stretch text-[var(--grey-800,#292929)]`} value="Computer Science">Computer Science</SelectItem>
                        <SelectItem className={`${fontStyles.bodyLargePrimary}self-stretch text-[var(--grey-800,#292929)]`} value="Software Engineering">Software Engineering</SelectItem>
                        <SelectItem className={`${fontStyles.bodyLargePrimary}self-stretch text-[var(--grey-800,#292929)]`} value="Electrical Engineering">Electrical Engineering</SelectItem>
                        <SelectItem className={`${fontStyles.bodyLargePrimary}self-stretch text-[var(--grey-800,#292929)]`} value="Other">Other</SelectItem>
                      </SelectGroup>
                    </SelectContent>
                  </Select>
